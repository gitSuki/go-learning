...
func (q *Queries) ListTransfersInvolvingAccount(ctx context.Context, arg ListTransfersInvolvingAccountParams) ([]Transfer, error) {
	rows, err := q.db.QueryContext(ctx, listTransfersInvolvingAccount, arg.FromAccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateTransferToAccount = `-- name: UpdateTransferToAccount :one
UPDATE transfers 
SET to_account_id = $2
WHERE id = $1
RETURNING id, from_account_id, to_account_id, amount, created_at
...
func (q *Queries) UpdateTransferToAccount(ctx context.Context, arg UpdateTransferToAccountParams) (Transfer, error) {
	row := q.db.QueryRowContext(ctx, updateTransferToAccount, arg.ID, arg.ToAccountID)
	var i Transfer
	err := row.Scan(
		&i.ID,
		&i.FromAccountID,
